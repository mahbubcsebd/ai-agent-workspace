...
- **Security & Project Isolation**: ONLY interact with web pages and tabs that belong to the active local development project (e.g., `http://localhost`, `http://127.0.0.1`, project-specific test domains, or associated external authentication/OAuth redirect flows like Auth0, Clerk, Firebase, Google, or GitHub login pages). Under no circumstances should you inspect, read, or interact with tabs or pages belonging to unrelated personal or sensitive domains (such as your personal social media, personal emails, bank accounts, or password managers). If a task requires visiting an unrelated external domain, explain to the user that it is restricted for security.
